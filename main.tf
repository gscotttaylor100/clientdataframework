...
resource "azurerm_databricks_workspace" "dbwdata01" {
  ...
  resource_group_name         = data.azurerm_resource_group.rgdata01.name
  location                    = data.azurerm_resource_group.rgdata01.location
  managed_resource_group_name = "databricks-rg-gst_x_005"
  sku                         = "premium"
...
resource "azurerm_key_vault" "kvdatabricks" {
  ...
  access_policy {
    ...
    key_permissions     = ["Get", "List", "Update", "Create", "Import", "Delete", "Recover", "Backup", "Restore", "UnwrapKey", "WrapKey", "Purge", "Encrypt", "Decrypt", "Sign", "Verify", "Release", "Rotate", "GetRotationPolicy", "SetRotationPolicy"]
